...
export default function Navbar() {
  ...
  const openResume = () => { setHireMeOpen(false); setResumeOpen(true); };
    ...
    <>
      ...
      >
        <nav className="max-w-7xl mx-auto px-6 h-[72px] flex items-center justify-between">
          <a href="#" className="font-mono text-sm font-semibold text-fg tracking-widest uppercase hover:text-accent-light transition-colors">
            FM
          </a>
          ...
          <ul className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <li key={link.href}>
                <a
                  href={link.href}
                  className="text-base font-sans text-fg-2 hover:text-fg transition-colors duration-200 relative group"
                >
                  {link.label}
          ...
          {menuOpen && (
            ...
            >
              <ul className="flex flex-col px-6 py-4 gap-4">
                ...
                <li>
                  <button
                    onClick={() => { setMenuOpen(false); setHireMeOpen(true); }}
      ...
      <HireMeModal isOpen={hireMeOpen} onClose={() => setHireMeOpen(false)} onOpenResume={openResume} />

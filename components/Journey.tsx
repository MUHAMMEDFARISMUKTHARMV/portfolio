...
const STEPS: JourneyStep[] = [
  ...
  },
  {
    ...
    subtitle: "MSc Specialisation",
    color: "text-violet-400",
    glow: "shadow-violet-500/20",
    milestone: "First ML research project",
  },
  {
    icon: Layers,
    title: "ML Research",
    subtitle: "LayoutLM · OCR · NLP",
    color: "text-accent-light",
    glow: "shadow-accent/20",
    milestone: "First production NLP model",
  },
  {
    icon: Brain,
    title: "ML Engineering",
    subtitle: "YOLOv8 · RoBERTa · LangChain",
    color: "text-emerald-400",
    glow: "shadow-emerald-500/20",
    milestone: "First YOLO deployment",
  },
  {
    icon: Cpu,
    title: "Edge AI",
    subtitle: "RPi · Jetson · ESP32",
    color: "text-orange-400",
    glow: "shadow-orange-500/20",
    milestone: "First edge deployment",
  },
  {
    icon: Globe,
    title: "Full-Stack AI",
    subtitle: "Next.js · AWS · Docker",
    color: "text-pink-400",
    glow: "shadow-pink-500/20",
    milestone: "First AWS hosted app",
  },
];
    ...
    <div className="flex gap-4" ref={ref}>
      <div className="flex flex-col items-center gap-0">
        <motion.div
          ...
          className="w-10 h-10 rounded-xl border border-white/10 bg-surface-2 flex items-center justify-center flex-shrink-0"
        >
          <Icon size={18} className={step.color} />
        </motion.div>
        {!isLast && <div className="w-px h-8 bg-white/8 my-1" />}
      </div>
      <motion.div
        ...
        className="pb-6"
      >
        <div className="font-sans font-semibold text-white text-sm">{step.title}</div>
        <div className="font-mono text-xs text-[#6b7280] mt-0.5">{step.subtitle}</div>
        {step.milestone && (
  ...
  return (
    <div ref={ref} className="flex flex-col items-center flex-1 min-w-0 relative">
      ...
      <motion.div
        ...
        className={`relative z-10 w-14 h-14 rounded-2xl border border-white/10 bg-surface-2 flex items-center justify-center shadow-xl ${step.glow}`}
      >
        <Icon size={22} className={step.color} />
        {/* Glow dot */}
        <div
          className={`absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full border border-bg ${step.color.replace("text-", "bg-")}`}
        />
      </motion.div>
      ...
      >
        <div className="font-sans font-semibold text-white text-sm leading-snug">
          {step.title}
        </div>
        <div className="font-mono text-xs text-[#6b7280] mt-0.5 leading-snug">
          {step.subtitle}
        </div>
      </motion.div>
      ...
      {step.milestone && (
        ...
        >
          <span className="font-mono text-[10px] text-accent-light leading-none">
            {step.milestone}
          </span>
        </motion.div>
      )}
  ...
  return (
    <div ref={ref} className="flex-shrink-0 flex items-start pt-7 w-8 sm:w-12">
      <motion.div
        ...
        style={{ originX: 0 }}
        className="h-px w-full bg-gradient-to-r from-white/15 to-white/5"
      />
    </div>
  );
}
    ...
    <section id="journey" className="py-24 px-6">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <p className="mt-4 font-sans text-sm text-[#6b7280] max-w-xl mx-auto">
        ...
        <div className="hidden sm:flex items-start justify-between gap-0 relative">
          {/* Background track */}
          <div className="absolute top-7 left-0 right-0 h-px bg-white/5 pointer-events-none" />
